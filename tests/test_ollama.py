...
@respx.mock
async def test_stream_summarize_yields_ndjson_tokens():
    ndjson = (
...
@respx.mock
async def test_stream_summarize_stops_on_done_flag():
    ndjson = (
...
@respx.mock
async def test_stream_summarize_skips_invalid_json_lines():
    ndjson = (
        b"this is not json\n"
...
@respx.mock
async def test_stream_summarize_skips_blank_lines():
    ndjson = (
        b"\n"
...
@respx.mock
async def test_stream_summarize_sends_model_in_payload():
...
@respx.mock
async def test_stream_summarize_includes_transcript():
    route = respx.post(f"{URL}/api/chat").mock(
        return_value=httpx.Response(200, content=json.dumps({"done": True}).encode() + b"\n")
    )
    svc = OllamaService(base_url=URL)
    [_ async for _ in svc.stream_summarize("UNIQUE_MARKER")]
...
@respx.mock
async def test_stream_summarize_404_with_pull_hint():
...
@respx.mock
async def test_stream_summarize_400_surfaces_body():
